...
class UserQueryInvokerJob implements ShouldQueue, QueryJobContract
{
    use Dispatchable;
    ...
    public function handle(
        ...
        try {
            $promptData = $this->generatePrompt($entities['tenant']);
        ...
        try {
            $result = $executionService->execute($config);
        ...
        try {
            if ($executionService->wasCsvAlreadyDelivered($this->queryId)) {
    ...
    /**
     * Generate prompt using domain command
     */
    private function generatePrompt(Tenant $tenant): array
    {
        $promptCommand = new GenerateInitialPromptCommand($this->queryId, $tenant->id);
     ...
     */
    private function notifySlack(Thread $thread, string $responseText, QueryStatus $finalStatus): void
    {
        $defaultErrorMessage = "I apologize, but I couldn't generate a SQL query for your request. Please try rephrasing your question.";
